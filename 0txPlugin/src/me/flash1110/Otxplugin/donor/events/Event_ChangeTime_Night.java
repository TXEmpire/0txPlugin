...
public class Event_ChangeTime_Night extends SpendEvent {
	
	Integer slot;
	
	Long LastUsed;
	
	public Event_ChangeTime_Night(Integer slot, Integer cost) {
		super (slot, cost);
		LastUsed = (long) 0;
	}
	...
	public void run(Player player) {
		...
		OtxPlugin.BroadcastMessage(ChatColor.YELLOW + "Time is set to" + ChatColor.LIGHT_PURPLE + " Night " + ChatColor.YELLOW + "by " + ChatColor.GRAY + player.getDisplayName());
		...
		if (player.getWorld().getEnvironment() == Environment.NORMAL) {
			...
			if (player.getWorld().getTime() <= 12000) {
				...
				if (difference < 30000) {
					...
					player.sendMessage(ChatColor.RED + "Time was recently set to night " + ChatColor.YELLOW + secondsgone + ChatColor.RED + " second ago");
				...
				player.sendMessage(ChatColor.RED + "It is already night within your world");
			}
